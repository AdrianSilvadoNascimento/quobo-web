  ...
  const handleGoogleAccept = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        setFormError('Falha ao iniciar autenticação com Google. Tente novamente.');
      }
    } catch {
      localStorage.removeItem('pending_invite_token');
      ...
      case 'valid':
        return (
          <>
            ...
            <div className="divider my-4 text-slate-400 text-sm">ou</div>

            <button
              type="button"
              disabled={isSubmitting}
              onClick={handleGoogleAccept}
              className="cursor-pointer w-full bg-white hover:bg-slate-50 text-slate-700 font-semibold py-3.5 rounded-md shadow-sm hover:shadow-md flex items-center justify-center gap-3 transition-all duration-300 transform hover:scale-[1.02] active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed border border-slate-200"
            >
              {isSubmitting ? (
                <span className="loading loading-dots loading-md"></span>
              ) : (
